...
The payment channel methodology proposed in this document requires one very minor addition to the existing BitShares core protocol, which is described separately in [BSIP-0069: Additional Assert Predicates](bsip-0069.md).  Aside from that one change, the methodology is based entirely upon existing features of the BitShares core protocol.  The needed protocol addition is an "assert predicate" which would allow a transaction to assert on a given head block time, chosen to be in the future at the time of transaction creation.  This allows a "time lock" to be placed on a transaction, and this, in turn, makes possible the payment channel protocol.  We describe a "short-lived" payment channel methodology, where the maximum lifetime of the channel is constrained by the core protocol requirment that a transaction include a "TaPoS" parameter identifying a reference block occuring within the last 2^16 blocks at time of transaction broadcast.  This sets a maximum possible channel lifetime at approximately two days (with useful channels being a bit shorter, to allow a window in which to broadcast the time-locked transaction).  By comparisson, a "long-lived" payment channel protocol could additionally be developed, but this would require much more significant protocol upgrades to enable functionality.  Thus a primary advantage of short-lived channels is that they can be implemented now (pending BSIP-0069).  Furthermore, we note that there are in fact interesting use cases wherein a two-day lifetime constraint is not limiting.  In particular, these channels may be used to implement ILP-style streaming micropayments, while keeping on-chain transactions to a minimum. 
...
The methodology builds upon existing core protocol features and requires no significant upgrades to the core protocol beyond a minor and uncontroversial upgrade to the 'assert' operation detailed in [BSIP-0069](bsip-0069.md).
